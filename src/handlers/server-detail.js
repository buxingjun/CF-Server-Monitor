...
export async function handleServerDetail(request, env, sys, viewId) {
  ...
  if (!isLoggedIn) query += " AND is_hidden != '1'";
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .terminal-title { color: var(--text-primary); font-weight: 600; }
    ...
    .time-btn.active { background: var(--accent-green); color: #000; font-weight: 600; }
    ...
    .host-name .prompt { color: var(--text-muted); margin-right: 4px; }
    ...
    .status-badge.online { background: rgba(0, 212, 170, 0.1); color: var(--accent-green); border-color: rgba(0, 212, 170, 0.3); }
    .status-badge.offline { background: rgba(248, 81, 73, 0.1); color: var(--accent-red); border-color: rgba(248, 81, 73, 0.3); }

    .pulse-dot { display: inline-block; width: 6px; height: 6px; border-radius: 50%; animation: pulse 2s infinite; }
    .pulse-dot.online { background: var(--accent-green); box-shadow: 0 0 6px var(--accent-green); }
    .pulse-dot.offline { background: var(--accent-red); box-shadow: 0 0 6px var(--accent-red); }
    ...
    .sysinfo-item:hover { background: var(--bg-hover); }
    ...
    .sysinfo-value.highlight { color: var(--accent-cyan); }
    ...
    .chart-card:hover { border-color: var(--border-active); box-shadow: 0 4px 20px rgba(0, 0, 0, 0.3); }
    ...
    .chart-title-icon { color: var(--accent-cyan); font-size: 14px; }
    .chart-current-value { font-size: 14px; font-weight: 700; color: var(--accent-green); text-shadow: 0 0 8px rgba(0, 212, 170, 0.3); }
    .chart-subtitle { font-size: 10px; color: var(--text-muted); margin-top: 2px; }
    .chart-body { padding: 16px; position: relative; }
    .chart-body canvas { width: 100% !important; height: 200px !important; }
    .chart-card.full-width .chart-body canvas { height: 280px !important; }

    .net-indicator { display: flex; gap: 16px; font-size: 12px; }
    .net-down { color: var(--accent-green); }
    .net-up { color: var(--accent-blue); }
    ...
    .status-bar-dot { width: 6px; height: 6px; border-radius: 50%; background: var(--accent-green); }
  ...
  <style id="detail-font-override">
    * { font-family: "Microsoft YaHei", "PingFang SC", "Noto Sans CJK SC", "JetBrains Mono", monospace !important; }
    ...
    .terminal-header { font-size: 16px !important; padding: 14px 20px !important; }
    .terminal-title { font-size: 18px !important; }
    .nav-bar { padding: 16px 20px !important; }
    .back-btn, .time-btn { font-size: 16px !important; padding: 10px 18px !important; }
    .host-card-header { padding: 20px 24px !important; }
    .host-name { font-size: 23px !important; }
    .status-badge { font-size: 16px !important; padding: 6px 14px !important; }
    .pulse-dot { width: 9px !important; height: 9px !important; }
    .sysinfo-grid { grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)) !important; }
    .sysinfo-item { padding: 18px 20px !important; }
    .sysinfo-label { font-size: 16px !important; margin-bottom: 8px !important; }
    .sysinfo-value { font-size: 18px !important; }
    #val-cpuinfo, #val-boot { font-size: 16px !important; }
    .charts-container { gap: 20px !important; }
    .chart-card-header { padding: 18px 22px !important; }
    .chart-title { font-size: 19px !important; }
    .chart-title-icon { font-size: 18px !important; }
    .chart-current-value { font-size: 24px !important; }
    .chart-subtitle { font-size: 16px !important; }
    .net-indicator { font-size: 17px !important; gap: 20px !important; }
    .chart-body { padding: 20px !important; }
    .chart-body canvas { height: 280px !important; }
    .chart-card.full-width .chart-body canvas, #chartPing { height: 380px !important; }
    .status-bar { font-size: 16px !important; padding: 14px 20px !important; }

    @media (max-width: 768px) {
      body { font-size: 16px !important; }
      .container { padding: 8px !important; }
      .host-name { font-size: 19px !important; }
      .back-btn, .time-btn { font-size: 14px !important; padding: 8px 12px !important; }
      .sysinfo-grid { grid-template-columns: 1fr !important; }
      .sysinfo-label { font-size: 14px !important; }
      .sysinfo-value { font-size: 16px !important; }
      .chart-title { font-size: 16px !important; }
      .chart-current-value { font-size: 20px !important; }
      .chart-body canvas { height: 230px !important; }
    }
  </style>
</head>
<body class="${sys.theme || 'theme1'}">
  <div class="container">
    <div class="terminal-header">
      ...
      <div class="terminal-title">${server.name}</div>
      <div></div>
    </div>
    ...
    <div class="host-card">
      ...
      <div class="sysinfo-grid" id="info-panel">
        <div class="sysinfo-item"><span class="sysinfo-label">⏱ Uptime</span><span class="sysinfo-value" id="val-uptime">${server.uptime || 'N/A'}</span></div>
        <div class="sysinfo-item"><span class="sysinfo-label">🏗 Architecture</span><span class="sysinfo-value" id="val-arch">${server.arch || 'N/A'}</span></div>
        <div class="sysinfo-item"><span class="sysinfo-label">💻 OS</span><span class="sysinfo-value" id="val-os">${server.os || 'N/A'}</span></div>
        <div class="sysinfo-item"><span class="sysinfo-label">🔧 CPU Model</span><span class="sysinfo-value" id="val-cpuinfo">${(server.cpu_info || 'N/A').substring(0, 40)}</span></div>
        <div class="sysinfo-item"><span class="sysinfo-label">⚙️ CPU Cores</span><span class="sysinfo-value" id="val-cpucores">${server.cpu_cores || 'N/A'}</span></div>
        <div class="sysinfo-item"><span class="sysinfo-label">📊 Load Average</span><span class="sysinfo-value highlight" id="val-load">${server.load_avg || '0.00'}</span></div>
        <div class="sysinfo-item"><span class="sysinfo-label">🕐 Boot Time</span><span class="sysinfo-value" id="val-boot">${server.boot_time || 'N/A'}</span></div>
        <div class="sysinfo-item"><span class="sysinfo-label">💾 Total RAM</span><span class="sysinfo-value" id="val-ram-total">${(parseFloat(server.ram_total || 0)/1024).toFixed(1)} GiB</span></div>
        <div class="sysinfo-item"><span class="sysinfo-label">💿 Total Disk</span><span class="sysinfo-value" id="val-disk-total">${(parseFloat(server.disk_total || 0)/1024).toFixed(1)} GiB</span></div>
        <div class="sysinfo-item"><span class="sysinfo-label">🔽 Traffic In</span><span class="sysinfo-value" id="val-traffic-in">${formatBytes(server.net_rx)}</span></div>
        <div class="sysinfo-item"><span class="sysinfo-label">🔼 Traffic Out</span><span class="sysinfo-value" id="val-traffic-out">${formatBytes(server.net_tx)}</span></div>
        <div class="sysinfo-item"><span class="sysinfo-label">⏰ Last Report</span><span class="sysinfo-value" id="val-last-report">${new Date(serverLastUpdated).toLocaleString(undefined, { hour12: false })}</span></div>
      </div>
    </div>

    <div class="charts-container">
      <div class="chart-card">
        <div class="chart-card-header"><span class="chart-title"><span class="chart-title-icon">▸</span>CPU Usage</span><span class="chart-current-value" id="text-cpu">${server.cpu || '0'}%</span></div>
        <div class="chart-body"><canvas id="chartCPU"></canvas></div>
      </div>
      <div class="chart-card">
        <div class="chart-card-header">
          <span class="chart-title"><span class="chart-title-icon">▸</span>Memory Usage</span>
          <div><span class="chart-current-value" id="text-ram">${server.ram || '0'}%</span><div class="chart-subtitle" id="text-swap">Swap: ${server.swap_used || '0'} / ${server.swap_total || '0'} MiB</div></div>
        </div>
        <div class="chart-body"><canvas id="chartRAM"></canvas></div>
      </div>
      <div class="chart-card">
        <div class="chart-card-header">
          <span class="chart-title"><span class="chart-title-icon">▸</span>Disk Usage</span>
          <div><span class="chart-current-value" id="text-disk">${server.disk || '0'}%</span><div class="chart-subtitle" id="text-disk-detail">Used ${(parseFloat(server.disk_used || 0)/1024).toFixed(2)} / ${(parseFloat(server.disk_total || 0)/1024).toFixed(2)} GiB</div></div>
        </div>
        <div class="chart-body"><canvas id="chartDisk"></canvas></div>
      </div>
      <div class="chart-card">
        <div class="chart-card-header"><span class="chart-title"><span class="chart-title-icon">▸</span>Network Traffic</span><div class="net-indicator"><span class="net-down">▼ <span id="text-net-in">${formatBytes(server.net_in_speed)}/s</span></span><span class="net-up">▲ <span id="text-net-out">${formatBytes(server.net_out_speed)}/s</span></span></div></div>
        <div class="chart-body"><canvas id="chartNet"></canvas></div>
      </div>
      <div class="chart-card">
        <div class="chart-card-header"><span class="chart-title"><span class="chart-title-icon">▸</span>Processes</span><span class="chart-current-value" id="text-proc">${server.processes || '0'}</span></div>
        <div class="chart-body"><canvas id="chartProc"></canvas></div>
      </div>
      <div class="chart-card">
        <div class="chart-card-header"><span class="chart-title"><span class="chart-title-icon">▸</span>Connections</span><div class="net-indicator"><span style="color: var(--accent-purple);">TCP <b id="text-tcp">${server.tcp_conn || '0'}</b></span><span style="color: var(--accent-pink);">UDP <b id="text-udp">${server.udp_conn || '0'}</b></span></div></div>
        <div class="chart-body"><canvas id="chartConn"></canvas></div>
      </div>
      <div class="chart-card full-width">
        <div class="chart-card-header">
          <span class="chart-title"><span class="chart-title-icon">▸</span>Latency Monitor</span>
        ...
        <div class="chart-body"><canvas id="chartPing"></canvas></div>
      </div>
    </div>

    <div class="status-bar">
      <div class="status-bar-item"><span class="status-bar-dot"></span><span>Last update: <span id="last-update">just now</span></span></div>
      <div class="status-bar-item"><span>Auto-refresh: 60s (status)</span></div>
    </div>
  ...
  <script>
    ...
    function translateDetailUI() {
      ...
      const walker = document.createTreeWalker(
        ...
        NodeFilter.SHOW_TEXT,
        {
          acceptNode(node) {
            ...
            if (node.parentElement && ['SCRIPT', 'STYLE', 'TEXTAREA', 'CANVAS'].includes(node.parentElement.tagName)) return NodeFilter.FILTER_REJECT;
      ...
      nodes.forEach(node => { node.nodeValue = translateDetailText(node.nodeValue); });
    }
    ...
    const CHART_FONT_FAMILY = "'Microsoft YaHei', 'PingFang SC', 'Noto Sans CJK SC', 'JetBrains Mono', monospace";
    ...
    Chart.defaults.plugins.tooltip.titleFont = { size: 17, weight: 'bold', family: CHART_FONT_FAMILY };
    Chart.defaults.plugins.tooltip.bodyFont = { size: 16, family: CHART_FONT_FAMILY };
    ...
    const createChartOptions = (unit = '', showLegend = false, yAxisLabel = '') => ({
      ...
      plugins: {
        legend: {
          ...
          labels: {
            boxWidth: 12,
            padding: 16,
            font: { size: 15, family: CHART_FONT_FAMILY },
            usePointStyle: true,
            color: '#8999af'
          }
        },
        tooltip: {
          callbacks: {
            ...
            label: function(context) {
              ...
              if (value !== null && value !== undefined) label += typeof value === 'number' ? value.toFixed(2) : value;
              return '$ ' + label + unit;
            }
          }
        }
      ...
      scales: {
        x: {
          ...
          min: 'dataMin',
          max: 'dataMax',
          ticks: { maxTicksLimit: 8, color: '#5c6d82', font: { size: 14, family: CHART_FONT_FAMILY }, maxRotation: 0, padding: 8 },
          grid: { color: 'rgba(30, 42, 58, 0.5)', drawBorder: false, tickLength: 0 }
        },
        y: {
          beginAtZero: true,
          title: { display: !!yAxisLabel, text: yAxisLabel, color: '#5c6d82', font: { size: 14, family: CHART_FONT_FAMILY } },
          grid: { color: 'rgba(30, 42, 58, 0.5)', drawBorder: false, tickLength: 0 },
          ticks: { color: '#5c6d82', font: { size: 14, family: CHART_FONT_FAMILY }, padding: 8, callback: function(value) { return value + unit; } }
        }
      },
      elements: {
    ...
    charts.ram = new Chart(document.getElementById('chartRAM').getContext('2d'), {
      type: 'line',
      data: { datasets: [{ label: t('Memory'), data: [], borderColor: '#b392f0', backgroundColor: 'rgba(179, 146, 240, 0.05)', fill: true, borderWidth: 1.5 }] },
      options: createChartOptions('%')
    });

    charts.disk = new Chart(document.getElementById('chartDisk').getContext('2d'), {
      type: 'line',
      data: { datasets: [{ label: t('Disk'), data: [], borderColor: '#39d2c0', backgroundColor: 'rgba(57, 210, 192, 0.05)', fill: true, borderWidth: 1.5 }] },
      options: createChartOptions('%')
    });

    charts.proc = new Chart(document.getElementById('chartProc').getContext('2d'), {
      type: 'line',
      data: { datasets: [{ label: t('Processes'), data: [], borderColor: '#f778ba', backgroundColor: 'rgba(247, 120, 186, 0.03)', fill: true, borderWidth: 1.5 }] },
    ...
    charts.net = new Chart(document.getElementById('chartNet').getContext('2d'), {
      ...
      data: {
        datasets: [
          { label: t('Download'), data: [], borderColor: '#00d4aa', backgroundColor: 'rgba(0, 212, 170, 0.03)', fill: true, tension: 0.4, borderWidth: 1.5, pointRadius: 0, hoverRadius: 5 },
          { label: t('Upload'), data: [], borderColor: '#4da6ff', backgroundColor: 'rgba(77, 166, 255, 0.03)', fill: true, tension: 0.4, borderWidth: 1.5, pointRadius: 0, hoverRadius: 5 }
        ]
      },
    ...
    function updateChartDataset(chart, datasetIndex, dataPoints, xField = 'timestamp', yField) {
      ...
      const processedData = sampledData.map(d => ({ x: new Date(d[xField]).getTime(), y: parseFloat(d[yField]) || 0 }));
      processedData.sort((a, b) => a.x - b.x);
      const completeData = [];
      if (processedData.length > 0 && processedData[0].x > startTime) completeData.push({ x: startTime, y: null });
      completeData.push(...processedData);
      if (processedData.length > 0) {
        const lastTimestamp = processedData[processedData.length - 1].x;
        const now = Date.now();
        if (lastTimestamp < now) completeData.push({ x: now, y: null });
      }
    ...
    function sampleDataByInterval(data, targetInterval) {
      ...
      for (const item of data) {
        ...
        if (ts - lastTs >= targetInterval) { result.push(item); lastTs = ts; }
      }
      return result;
    }
      ...
      try {
        ...
        if (hours <= 1) {
          ...
          if (!oneHourDataCache) {
            ...
            if (oneHourRes.ok) oneHourDataCache = await oneHourRes.json();
          }
    ...
    async function fetchCurrentStatus() {
      try {
        ...
        document.getElementById('val-cpuinfo').innerText = (data.cpu_info || 'N/A').substring(0, 40);
        ...
        document.getElementById('val-ram-total').innerText = (parseFloat(data.ram_total || 0)/1024).toFixed(1) + ' GiB';
        document.getElementById('val-disk-total').innerText = (parseFloat(data.disk_total || 0)/1024).toFixed(1) + ' GiB';
